...
@safe nothrow @nogc:
    ...
    void toString(scope ref StringBuilder_UTF8 builder) @trusted {
        if(isNull)
            builder ~= "ConcurrentHashMapImpl!(" ~ KeyType.stringof ~ ", " ~ ValueType.stringof ~ ")@null";
        else
            builder.formattedWrite("ConcurrentHashMapImpl!(" ~ KeyType.stringof ~ ", " ~ ValueType.stringof ~ ")@{:p}(length={:d}",
